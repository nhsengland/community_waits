...
These predictions are based purely on historic demand and capacity and do not take into account seasonal variation or other features. They are based upon a stochastic application of referrals and removal distributions. 
:::
...
If the load on a waiting list is less than 1 then the chance of missing the target halves each time we increase the target by some fixed number of days.
...
Scenario modelling was outside the scope of this analysis, but could be added into the workflow at a later stage.

The model is open source and can easily be adapted based on more assured local data sources.  The code to create the model is open source and could be pointed at a different data source in order to get more localised assurance of the data.  
