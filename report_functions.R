...
provider_model_table <- function(org, waits)  {
  ...
  if(waits == '52') { 
    model_table <- tab_data |> 
      ...
      tab_style(
        style = cell_text(color = "#DA291C", weight = 'bolder', size = 'large'),
        locations = cells_body(
          columns = wl_load,
          rows = wl_load >=1.05
        )
...
# provider_model_table ('LIVEWELL SOUTHWEST', 52)

# commentary function

commentary <- function(org, waits) {
        ...
        tar_capacity  )} else {
          df_model |>
            ...
            select(
              ...
              total_waits, # total waits
              target_queue_size_18, # total waits
              queue_ratio = queue_ratio_18, # total waits
              perc_18, # performance
              wl_pressure = wl_pressure_18, # performance
              mean_removals_dup,# capacity
              additional_capacity =additional_capacity_18, # capacity
              tar_capacity = tar_capacity_18) # capacity 
        }
  

  
  services_exceptional_load <- tab_data$service[tab_data$wl_load >= 2]
  
  tx_exp_load <- if(is_empty(services_exceptional_load)) {'There are no services under exceptional load.'} else {
    paste0('The following service(s) are showing as being under exceptional load **', toString(services_exceptional_load),
           '**, which would suggest a service where waits are going to significantly increase without further intervention')
  }
  
  que_exceptional_load <- tab_data$service[tab_data$queue_ratio >= 2]
  que_high <- tab_data$service[tab_data$queue_ratio >= 2 & tab_data$queue_ratio < 10]
  
  tx_que_ratio_exp <- if(is_empty(que_exceptional_load)) {'There are no services with exceptional queue ratios.'} else {
    paste0('The following service(s) are showing as having exceptional queue ratios **', toString(que_exceptional_load),
           '**, which would suggest a service where the queue size is significantly higher than the suggested size to achieve target.')
  }
  
  press_exceptional <- tab_data$service[tab_data$wl_pressure >= 2]
  
  tx_que_pressure_exp <- if(is_empty(press_exceptional)) {'There are no services with exceptional queue pressure.'} else {
    paste0('The following service(s) are showing as having exceptional queue pressure **', toString(press_exceptional),
           '**, which would suggest a service where waiting times are significantly unlikely to achieve target based on a comparison of current waits to target wait.')
  }
  
  space <- '\n\n'
  
  comments <- paste0(tx_exp_load, space, tx_que_ratio_exp, space, tx_que_pressure_exp, space)
  
  cat(comments)
  
}

#commentary('CORNWALL PARTNERSHIP', 18)
...
plot_predictions_trajectory <- function(org, serv, traj = F, wait) {
  ...
  plot <- com |>
    ...
    geom_textpath(aes(x = dates, y = mean_q), 
                  ...
                  hjust = 0.3, 
                  straight = TRUE) +
    geom_ribbon(data = traject_cht, aes(x = dates, ymin = q_25, ymax = q_75), alpha = 0.5, fill = "lightblue") +
    theme_minimal() +
    labs(
      title = paste0(org, "\n", serv),
      x = NULL,
    ...
    labs(      subtitle = paste0("Projected waiting list size to ", 
