...
tab_data <- if(waits == '52') 
  ...
  tab_header(
    title = paste0(org, ": Community Services waits as at ", format(latest_date, "%B %y"), " : Analysis to achieve zero at 52 weeks target"),
  ...
  tab_header(
    title = paste0(org, ": Community Services waits as at ", format(latest_date, "%B %y"), " : Analysis to achieve 92% of patients waiting less than 18 weeks"),
    subtitle = md("Waiting lists by service with waiting list metrics")
  ) |>
  tab_footnote(
    footnote = md(paste0(
...
trajectory <- trajectory |>
  mutate(traj = case_when(date == min(date) ~ total_waits,
    ...
    date == max(date) ~ target_queue_size_18,
    .default = NA
  )) |>
